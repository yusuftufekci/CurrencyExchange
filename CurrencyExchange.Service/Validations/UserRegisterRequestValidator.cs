...
namespace CurrencyExchange.Service.Validations
{
    public class UserRegisterRequestValidator : AbstractValidator<UserRegisterRequest>
    {
        public UserRegisterRequestValidator()
        {
            ...
            RuleFor(x => x.Password).NotEmpty().WithMessage("{PropertyName} can't be empty aaaaaaaaa}").Length(9, 15).WithMessage("Password must be atleast 9 character").NotNull().WithMessage("{PropertyName} can't be null");
